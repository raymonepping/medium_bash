...
# --- Top10 rule definitions + Bonus 0 ---
declare -a RULES
declare -a STATUS
declare -a MARKS
...
# 0: VERSION line presence (Bonus)
if grep -qE '^VERSION="' "$SCRIPT"; then
  STATUS[0]="${color_green}${STAR} bonus${color_reset}"
  MARKS[0]=":star: bonus"
else
  STATUS[0]="${color_red}${CROSS} bad${color_reset}"
  MARKS[0]=":x: bad"
fi
